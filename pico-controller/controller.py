...
# Configuration
ROVER="http://192.168.178.74:8080/"
...
async def connect_network(console):
    ...
    wlan.active(True)
    wlan.connect('amberdown', 'candleandthestar')
...
async def main():
    ...
    controls = JoystickController(ROVER)
    asyncio.create_task(controller.check_touch())
    asyncio.create_task(controller.display_loop("console"))
    asyncio.create_task(controls.watch_motor_joystick())
    await connect_network(controller.screens["console"])
